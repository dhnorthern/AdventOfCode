...
namespace DailyPuzzle.Day6
{
   ...
   {
      public string Name => "Customs Declarations";

      public void RunPart1()
      {
         Console.WriteLine( $"Sum of group yes counts: { Sum( Input, (answers, groupSize) => new AnswerCounter().CountUniqueAnswers( answers, groupSize ) ) }" );
      }

      public void RunPart2()
      {
         Console.WriteLine( $"Sum of group yes counts: { Sum( Input, (answers, groupSize) => new AnswerCounter().CountConsensus( answers, groupSize ) ) }" );
      }

      private static int Sum( IEnumerable<string> input, Func<string, int, int> counterFunction )
      {
         var groupYesCount = 0;
         ...
         foreach ( var line in input )
         {
            ...
            {
               var count = counterFunction( groupAnswers.ToString(), groupSize );
               Console.WriteLine( $"Group size: {groupSize}: {groupAnswers} {count}" );
               groupYesCount += count;
               groupAnswers.Clear();
               groupSize = 0;
            }
      ...
      private readonly List<string> Input = new List<string>
      {
         ...
         "", // injected end-of-record rather than introducing code to handle the absence of this marker
      };
